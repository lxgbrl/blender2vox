...
@dataclass
class VoxelData:
    """Container for voxel model data."""
    size_x: int
    size_y: int
    size_z: int
...
def read_vox_file(filepath: str) -> VoxelData:
    """Read a VOX file and return its voxel data.
    ...
    with open(filepath, 'rb') as f:
        ...
        if version != 150:
            print(f"Warning: VOX version {version} (expected 150)")
        ...
        # Read MAIN chunk
        main = read_chunk()
        if main is None or main[0] != 'MAIN':
        ...
        while True:
            ...
            chunk_id, content, children_size = chunk

            if chunk_id == 'SIZE':
                size_x, size_y, size_z = struct.unpack('<III', content[:12])
                ...
                # Parse palette (256 colors, indices 0-255)
                # In VOX format, palette[0-254] maps to color index [1-255]
                palette = []
                for i in range(256):
                    offset = i * 4
